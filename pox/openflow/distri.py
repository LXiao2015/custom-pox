...
def sendFlowTable(pathList):
    ...
    for path in pathList:
        ...
        for i in range(1, pathLen - 1):
            ...
            sendToSwitchByNodeNumber(path[i], path[i - 1], path[i + 1], path[pathLen - 1])
            sendToSwitchByNodeNumber(path[i], path[i + 1], path[i - 1], path[0])
...
def sendToSwitchByNodeNumber(sw, fr, to, dst):
    src_ip = "10.0.0." + fr
    dst_ip = "10.0.0." + dst
